...
function OrdersPageContent() {
  ...
  // Handle URL parameters for auto-opening review modal
  useEffect(() => {
    const orderId = searchParams.get('orderId')
    const openReview = searchParams.get('openReview')
    
    if (orderId && openReview === 'true' && orders.length > 0) {
      // Find the order and open review modal for the first product
      const order = orders.find(o => o._id === orderId)
      if (order && order.items.length > 0) {
        // Auto-open review modal for the first product in the order
        const firstProductId = extractId(order.items[0].productId)
        
        // Set modal context and open modal
        setModalContext({ orderId, productId: firstProductId })
        setIsReviewModalOpen(true)
            ...
            <div className="bg-gray-900 rounded-xl shadow-xl border border-gray-700 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-700">
                <h3 className="text-white font-semibold text-lg">
                  {modalContext && userReviews[getReviewKey(modalContext.orderId, modalContext.productId)] ? 'Edit Review' : 'Rate Your Experience'}
                </h3>
