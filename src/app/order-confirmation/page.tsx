  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      if (order.status !== 'delivered') {
        showToast('Invoice can only be downloaded for delivered orders', 'warning')
        return
      }

      if (order.paymentStatus !== 'paid') {
        showToast('Invoice can only be downloaded for paid orders', 'warning')
        return
      }
                ...
                {typeof order.shippingAddress === 'object' && order.shippingAddress ? (
                  <div className="space-y-1">
                    ...
                    {order.shippingAddress.secondaryPhone && (
                      <p className="text-gray-300">{order.shippingAddress.secondaryPhone}</p>
                    )}
                    <p className="text-gray-300">{order.shippingAddress.email}</p>
                    <p className="text-gray-300">{order.shippingAddress.address}</p>
                    <p className="text-gray-300">{order.shippingAddress.city}, {order.shippingAddress.state} {order.shippingAddress.zipCode}</p>
                    <p className="text-gray-300">{order.shippingAddress.region}, {order.shippingAddress.country}</p>
                  </div>
                ) : (
          ...
          <div className="lg:col-span-1">
            <div className="bg-gray-800 rounded-xl p-6 sticky top-8">
              ...
              <div className="space-y-3">
                {/* Only show download invoice button if order is delivered and paid */}
                {order.status === 'delivered' && order.paymentStatus === 'paid' ? (
                  ...
                  <div className="w-full bg-gray-700 text-gray-400 py-3 px-4 rounded-lg font-semibold flex items-center justify-center gap-2 cursor-not-allowed">
                    <Download className="w-5 h-5" />
