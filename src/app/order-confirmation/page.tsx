...
import { CheckCircle, Package, Truck, Home, Download, Share2, Loader2 } from 'lucide-react'
...
export default function OrderConfirmationPage() {
  ...
  const handleShareOrder = () => {
    ...
    if (navigator.share) {
      navigator.share({
        title: 'My Order',
        text: `Order ${order.orderNumber || order._id} - TechSouq`,
        url: window.location.href
      })
    } else {
      navigator.clipboard.writeText(`Order ${order.orderNumber || order._id} - TechSouq`)
  ...
  const estimatedDelivery = new Date(new Date(order.orderDate).getTime() + 3 * 24 * 60 * 60 * 1000)
            ...
            <div className="bg-gray-800 rounded-xl p-6">
              ...
              <div className="space-y-4">
                {order.items.map((item, index) => {
                  ...
                  console.log('Item productId:', item.productId) // Debug log
                  ...
                  console.log('Final imageUrl:', imageUrl) // Debug log
                  ...
                  return (
                    <div key={index} className="flex items-center gap-4 p-4 bg-gray-700 rounded-lg">
                      ...
                      <div className="text-right">
                        <p className="text-amber-400 font-bold">AED {item.price.toFixed(2)}</p>
                        <p className="text-gray-400 text-sm">Total: AED {item.total.toFixed(2)}</p>
                      </div>
                    </div>
                  )
                })}
            ...
            <div className="bg-gray-800 rounded-xl p-6 sticky top-8">
              ...
              <div className="space-y-3 mb-6">
                <div className="flex justify-between">
                  ...
                  <span className="text-white">AED {(order.subtotal || order.totalAmount).toFixed(2)}</span>
                </div>
                {order.tax !== undefined && order.tax > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-300">Tax (VAT 5%)</span>
                    <span className="text-white">AED {order.tax.toFixed(2)}</span>
                  </div>
                )}
                <div className="flex justify-between">
