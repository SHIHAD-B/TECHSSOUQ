...
export default function CheckoutPage() {
    ...
    try {
      ...
      const orderData = {
        ...
        shippingAddress: {
          ...
          secondaryPhone: selectedAddressData.secondaryPhone,
          email: selectedAddressData.email,
          address: selectedAddressData.address,
          city: selectedAddressData.city,
          state: selectedAddressData.state,
          region: selectedAddressData.region,
          zipCode: selectedAddressData.zipCode,
          country: selectedAddressData.country,
          type: selectedAddressData.type
        },
        shippingAddressString: `${selectedAddressData.address}, ${selectedAddressData.city}, ${selectedAddressData.state} ${selectedAddressData.zipCode}, ${selectedAddressData.country}`,
  ...
  const updateProductInventory = async (orderItems: any[]) => {
    try {
      // Supabase client is no longer needed, Appwrite user ID is used directly
      // The inventory update logic would typically involve a backend endpoint
      // that verifies the user's session and updates the inventory.
      // For now, we'll just log a message.

      // Example: If you had a backend endpoint for inventory updates, you would call it here
      // with the user's Appwrite session token.
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="text-center">
            ...
            <button
              onClick={() => router.push('/address?from=checkout')}
              className="bg-amber-600 hover:bg-amber-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors"
            >
              Add Address
            </button>
