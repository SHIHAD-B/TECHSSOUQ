...
export default function ProductPageClient({ params, initialProduct, initialError }: ProductPageClientProps) {
  ...
  // Fetch product data from API if not provided initially
  useEffect(() => {
    if (!initialProduct) {
      ...
      setActiveTab(initialProduct.showFeatures?'features':initialProduct.showSpecifications?'specifications':'reviews')
    }
  }, [params.id, initialProduct])

  // Ensure initially selected color is in stock
  useEffect(() => {
    if (product?.showColors && product.colors && product.colors.length > 0) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* Similar Products */}
        <SimilarProducts productId={params.id} currentProduct={product} />
      </div>
