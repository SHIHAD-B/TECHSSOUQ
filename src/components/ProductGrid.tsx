                ...
                <button
                  ...
                  className={`absolute top-2 left-2 p-2 rounded-full transition-all duration-300 ${
                    isFavorite(product._id)
                      ...
                      : 'bg-gray-800 bg-opacity-80 text-gray-300 border border-transparent hover:bg-red-500 hover:text-white'
                  } ${isFavoriteLoading ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <Heart className={`w-4 h-4 ${isFavorite(product._id) ? 'fill-current text-white' : ''}`} />
                </button>
              </div>
